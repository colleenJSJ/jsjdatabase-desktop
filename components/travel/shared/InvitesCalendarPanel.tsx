...
export function InvitesCalendarPanel({
  ...
  const showInviteToggle = typeof doNotSendInvite === 'boolean' && typeof onDoNotSendInviteChange === 'function';
      ...
      {showInviteToggle && (
        <label className="block text-sm">
          <input
            type="checkbox"
            className="mr-2"
            checked={Boolean(doNotSendInvite)}
            onChange={(e) => onDoNotSendInviteChange?.(e.target.checked)}
          />
          Do not send invite
        </label>
      )}
