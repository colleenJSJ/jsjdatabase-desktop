...
export default function DocumentUploadModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      debugLog('[Upload Modal] Starting upload...');
      debugLog('[Upload Modal] Cookies:', document.cookie);
      ...
      debugLog('[Upload Modal] Sending request to:', endpoint);
      ...
      debugLog('[Upload Modal] Response status:', response.status);
      debugLog('[Upload Modal] Response headers:', Object.fromEntries(response.headers.entries()));
              ...
              <div className="space-y-2">
                <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto">
                  {personOptions.map(option => {
                    const isSelected = selectedPersonIds.includes(option.id);
                    return (
                      <button
                        key={option.id}
                        type="button"
                        onClick={() => handlePersonToggle(option.id)}
