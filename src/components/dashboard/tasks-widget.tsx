...
export function TasksWidget() {
  ...
  const handleCompleteTask = async (taskId: string) => {
    try {
      console.log('[Dashboard] Marking task complete:', taskId);
      const response = await fetch(`/api/tasks/${taskId}/complete`, {
        method: 'POST',
        headers: addCSRFToHeaders(),
      });
      console.log('[Dashboard] Complete response:', response.status, response.ok);
      if (response.ok) {
        console.log('[Dashboard] Task marked complete, refetching tasks');
  ...
  const handleTogglePending = async (taskId: string, isPending: boolean) => {
    try {
      console.log('[Dashboard] Toggling pending status:', taskId, isPending);
      ...
      console.log('[Dashboard] Pending toggle response:', response.status, response.ok);
      if (response.ok) {
        console.log('[Dashboard] Task pending status toggled, refetching tasks');
        await fetchTasks();
      }
    } catch (error) {
