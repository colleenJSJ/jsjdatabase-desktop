...
export function CommentsDrawer({ open, onClose, onOpenTask }: { open: boolean; onClose: () => void; onOpenTask?: (taskId: string) => void }) {
  ...
  const [selected, setSelected] = useState<ThreadPreview | null>(null); // kept for backward compatibility (stream → thread), no longer used for Threads view
  ...
  const loadThread = async (th: ThreadPreview) => {
    setSelected(th);
    setLoading(true);
    try {
      const res = await fetch(`/api/tasks/comments/${th.thread_id}`);
      if (res.ok) {
        const data = await res.json();
    ...
    // Mark thread as read
    try {
      await fetch('/api/tasks/comments/read', {
        method: 'POST',
        headers: addCSRFToHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ thread_id: th.thread_id })
      });
      setThreads(prev => prev.map(t => t.thread_id === th.thread_id ? { ...t, unread_count: 0 } : t));
    } catch {}
  };
  ...
  const loadTask = async (task: { task_id: string; task_title: string; last_comment_at?: string }) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setComments(list);
        const threadIds = Array.from(new Set((list as any[]).map((c) => c.parent_comment_id || c.id).filter(Boolean)));
